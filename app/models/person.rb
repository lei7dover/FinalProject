class Person < ActiveRecord::Base
  ...
  scope :records_sport_pluck, -> {joins(:records).pluck("Distinct records.sport")}
  scope :records_sport_filter, -> (sport) { joins(:records).where("records.sport" => sport) }

  scope :people_last_name_pluck, -> {pluck("Distinct people.last_name")}
  scope :people_last_name_filter, -> (last_name) { where("people.last_name" => last_name) }

  scope :people_class_of_pluck, -> {pluck("Distinct people.class_of")}
  scope :people_class_of_filter, -> (class_of) { where("people.class_of" => class_of) }


  scope :records_all_state_1st_team, -> (all_state_1st_team) { where("records.all_state_1st_team" => all_state_1st_team) }
